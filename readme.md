...
#### (1):项目分工：
        ...
        林新祺：系统架构设计+完全分布式环境的集群部署+调度系统与RPC调用、爬虫节点（SpringBoot模块）的整合实现+URL库的存取操作+整合爬虫节以及部署
...
4.可升级部分：
                  zookeeper自动报警，监控报警系统的加入可以让让使用者可以主动发现节点宕机，而不是等出错后被动地发现，实际中爬虫程序可能是持续不断运行的，在多个节点上部署了爬虫程序，很有必要对节点进行监控，可以在节点出现问题时及时发现并修正。